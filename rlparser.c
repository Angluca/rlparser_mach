...
static void ExportParsedData(const char *fileName, int format)
{
    ...
    switch (format)
    {
        ...
        //./raylib_parser -i raylib.h -o raylib.mach -f CODE
        {
            ...
            // add base header
            if(GetFileBuf("input/_header.mach", _fileBuf, MAXLEN) < 1) {
                perror("read base_file errorr");
            }
            fprintf(outFile, "%s", _fileBuf);

            if(strstr(inFileName, "raylib.h")) {
                if(GetFileBuf("input/_raylib.base.mach", _fileBuf, MAXLEN) < 1) {
                    perror("read base_file errorr");
                }
                fprintf(outFile, "%s\n", _fileBuf);
            }
                ...
                if(strstr(inFileName, "rlgl.h")) {
                    if(GetFileBuf("input/_rlgl.base.mach", _fileBuf, MAXLEN) < 1) {
                        perror("read base_file errorr");
                    }
                    fprintf(outFile, "%s\n", _fileBuf);
                }
